...
class SISO_Capacity:
    ...
    def get_capacity_SNR(self, SNR_dB, sim_num):
        ...
        if self.syms_dim == 1:
            awgn = torch.randn(sim_num, 1, dtype=torch.float32, device=self.device)
        else:
            awgn = np.sqrt(1 / 2) * (
        ...
        if self.syms_dim == 1:
            metric = torch.exp(
                -abs(awgn + h * self.syms_dist * np.sqrt(SNR)).reshape(
                    sim_num, self.M, self.M
                )
                ** 2
                / 2
            )
        ...
        const = 0.5 * np.log2(np.exp(1)) if self.syms_dim == 1 else np.log2(np.exp(1))
        return np.log2(self.M) - tmp - const
