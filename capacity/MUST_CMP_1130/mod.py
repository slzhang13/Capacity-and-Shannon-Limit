...
def grayConverter(label1, label2, K1, K2):
    ...
    for i in range(0, K1, 2):
        label2_bin_converted[0] = int(
            not (label2_bin_converted[0] ^ label1_bin[i]))

    for i in range(1, K1, 2):
        label2_bin_converted[1] = int(
            not (label2_bin_converted[1] ^ label1_bin[i]))
    ...
    if MUST_type == 1:
        ...
        for i in range(M1):
            ...
            for j in range(M2):
                ...
                symbols[i*M2+j] = symbol1 * \
                    np.sqrt(P1)+np.sqrt(P2)*symbols2[label2_gray_converted]
