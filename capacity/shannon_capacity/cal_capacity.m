function capacity = cal_capacity(conste_norm, R, EbNo_dB, labels)

    M = length(conste_norm);
    K = log2(M);
    No = 1;
    EbNo_Lin = 10.^(0.1*EbNo_dB);
    capacity = zeros(size(EbNo_dB));
    ...
    if (nargin==4)
        ...
        for EbNo_cnt = 1:length(EbNo_dB)

            ebno_lin = EbNo_Lin(EbNo_cnt);
            Es = ebno_lin*No*K*R;
